...
def buildFileFullPath(log_file_path: str) -> str:
    home=expanduser('~')
    return home+'/'+'.alert_log.' + log_file_path.replace('/', '-').replace('\\', '_').replace(':','.')

def savePreAlerts(file_path: str, alerts: list):
...
def readFromFile(file_path: str) -> dict:
    alert_file_path = buildFileFullPath(file_path)
...
if __name__ == '__main__':
    ...
    alerts=findAlerts(file_path,'[2020-03-24 16：00：03.018',3)
    alerts1=readFromFile(file_path)
    print(alerts)
